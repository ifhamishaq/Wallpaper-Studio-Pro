...
-- Explicit FK for PostgREST joins (Fixes 400 error on community feed)
do $$
begin
  if not exists (select 1 from information_schema.table_constraints where constraint_name = 'wallpapers_user_id_fkey_profiles') then
    alter table public.wallpapers add constraint wallpapers_user_id_fkey_profiles foreign key (user_id) references public.profiles(id);
  end if;
end $$;
...
do $$
begin
    if not exists (select 1 from information_schema.columns where table_name = 'wallpapers' and column_name = 'views_count') then
        alter table public.wallpapers add column views_count integer default 0;
    end if;
end $$;

-- RPC to increment views atomically
create or replace function public.increment_wallpaper_views(p_wallpaper_id uuid)
returns void as $$
begin
